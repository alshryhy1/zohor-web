...
export type MomentsFeedProps = {
  initialMoments: Array<{ id: string; mediaUrl: string; desc: string; username: string; userId: string }>;
...
function emptyProps(meId: string, meUsername: string): MomentsFeedProps {
  return { initialMoments: [], initialFollowingIds: [], meId, meUsername };
}

export async function loadMomentsFeedProps(): Promise<MomentsFeedProps> {
  ...
  if (localMode) {
    ...
    return emptyProps(meId, meUsername);
  }
  ...
  let initialMoments = (data || [])
    ...
    .filter(Boolean) as MomentsFeedProps["initialMoments"];
  ...
  const candidateFollowIds = Array.from(
    new Set(
      initialMoments.map((m) => String(m.userId || "").trim()).filter((v) => v && meId && v !== meId)
    )
  );
  ...
  return {
    ...
    initialFollowingIds,
    meId,
    meUsername: meUsername || (meId ? "" : "زائر"),
  };
}
