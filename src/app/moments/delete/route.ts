...
export async function POST(req: Request) {
  try {
    ...
    if (!row) {
      return NextResponse.json({ ok: false, code: "not_found", message: "اللحظة غير موجودة." }, { status: 404 });
    }

    const ownerId = String((row as { user_id?: unknown } | null)?.user_id || "").trim();
    if (!ownerId) {
      return NextResponse.json(
        { ok: false, code: "missing_column", message: 'يلزم وجود user_id على اللحظة لتفعيل الحذف الآمن.' },
        { status: 500 }
      );
    }
