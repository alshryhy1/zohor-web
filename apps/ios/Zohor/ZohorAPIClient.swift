...
actor ZohorAPIClient {
    ...
    func registerAccount(email: String, password: String, name: String, username: String) async throws {
        ...
        let decoded = (try? JSONDecoder().decode(Response.self, from: data)) ?? Response(ok: nil, code: nil, message: nil)
        if decoded.ok != true {
        ...
        if writesViaBFF {
            do {
                ...
                _ = try await requestPlainJSON(
                    path: "/api/profile",
                    body: Body(username: username, displayName: displayName, avatarUrl: avatarUrl?.absoluteString)
                )
                return
            } catch {}
        }
    ...
    func listLiveHosts() async -> [LiveHost] {
        do {
            ...
            return await withIdentities(rows.compactMap(\.host))
        } catch {
            return []
        }
    }
    ...
    func liveChallenge(action: String = "get", userId: String? = nil, hostUserId: String? = nil) async -> LiveChallenge {
        (try? await liveChallengeThrowing(action: action, userId: userId, hostUserId: hostUserId)) ?? LiveChallenge()
    }

    func liveChallengeThrowing(action: String, userId: String? = nil, hostUserId: String? = nil, challengeId: String? = nil) async throws -> LiveChallenge {
        struct Body: Encodable { let action: String; let userId: String?; let hostUserId: String?; let challengeId: String? }
        ...
        let data = try await requestPlainJSON(path: "/live/challenge", body: Body(action: action, userId: userId, hostUserId: hostUserId, challengeId: challengeId))
    ...
    func agoraJoin(channel: String, role: String = "audience") async throws -> (appId: String, token: String, uid: UInt) {
        ...
        let uid = UInt.random(in: 1...999_999_999)
        let data = try await requestPlainJSON(
            path: "/api/agora/token",
            body: Body(channel: channel, uid: uid, role: role)
        )
        let decoded = try JSONDecoder().decode(Envelope.self, from: data)
        guard let token = decoded.token, !token.isEmpty, let appId = decoded.appId, !appId.isEmpty else {
            throw ZohorAPIError.server(code: "agora", message: decoded.message ?? "تعذر دخول البث.", status: 400)
        }
    ...
    func buyLiveCoins(packId: String) async throws -> Int {
        struct Body: Encodable { let action = "buy"; let packId: String }
        struct Envelope: Decodable { let coins: Int?; let message: String? }
        let data = try await requestPlainJSON(path: "/live/gifts", body: Body(packId: packId))
        let decoded = (try? JSONDecoder().decode(Envelope.self, from: data)) ?? Envelope(coins: nil, message: nil)
        return decoded.coins ?? 0
    }

    func sendLiveGift(giftKey: String, receiverId: String, challengeId: String) async throws -> (coins: Int, title: String) {
        struct Body: Encodable {
            let action = "send"
            let giftKey: String
    ...
    private func identity(for userId: String, username: String, in names: [String: PublicIdentity]) -> PublicIdentity? {
        ...
        if !id.isEmpty, let match = names[id] { return match }
    ...
    private func withIdentities(_ hosts: [LiveHost]) async -> [LiveHost] {
        let names = await publicIdentities(for: hosts.map(\.userId), usernames: hosts.map(\.username))
        return hosts.map { host in
            var next = host
            if let ident = identity(for: host.userId, username: host.username, in: names) {
                if next.username.isEmpty { next.username = ident.username }
                if !ident.displayName.isEmpty { next.displayName = ident.displayName }
                if let avatar = ident.avatarUrl { next.avatarUrl = avatar }
            }
            return next
    ...
    func identityCard(userId: String, username: String) async -> (displayName: String, avatarUrl: URL?) {
        let names = await publicIdentities(for: [userId], usernames: [username])
        if let ident = identity(for: userId, username: username, in: names) {
    ...
    private func mergeIdentity(_ profile: Profile, with identity: AuthIdentity) -> Profile {
        var next = profile
        if next.displayName?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty != false {
